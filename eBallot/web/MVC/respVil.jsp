    ...
    <body>
        <c:if test="${not empty param.update}">
            <sql:query var="result" dataSource="jdbc/eBallot">
                SELECT * FROM village where village_id = ? <sql:param value="${param.selected}"/>
            </sql:query>
            <c:redirect url="..\webpage\updateVillage.jsp">
                <c:forEach var="row" items="${result.rows}">
                    <c:param name="village_id" value="${param.selected}"/>
            ...
            <c:if test="${not empty param.delete}">
                <c:redirect url="..\webpage\villageDisplay.jsp">
                <sql:update var="del" dataSource="jdbc/eBallot">
                DELETE FROM village
                WHERE village_id = ? <sql:param value="${param.selected}"/>
            </sql:update>
                </c:redirect>
            </c:if>
